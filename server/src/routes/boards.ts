...
type isBoardType<Literal> = Literal extends BoardType ? true : false;
...
router.put("/:id/presence", (req: Request, res: Response): void => {
  ...
  const {
    ...
    lastName = "",
    photo100 = "",
  } = req.body as Partial<PresenceUser>;
  if (!boardPresence.has(id)) boardPresence.set(id, new Map());
  boardPresence
    .get(id)!
    .set(userId, { userId, firstName, lastName, photo100, ts: Date.now() });
  res.status(204).send();
});
...
router.get("/:id/presence", (req: Request, res: Response): void => {
  ...
  const users = Array.from(
    boardPresence.get(req.params.id as string)?.values() ?? [],
  )
